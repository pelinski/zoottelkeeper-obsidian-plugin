...
import { isInAllowedFolder, updateFrontmatter, updateIndexContent, removeFrontmatter, hasFrontmatter } from "./utils";
...
export default class ZoottelkeeperPlugin extends Plugin {
  ...
  getVaultSet() {
    const includedFiles = this.app.vault.getMarkdownFiles().filter((file) => {
      const filePath = file.path;
      return isInAllowedFolder(this.settings, filePath);
    });
    return new Set(includedFiles.map((file) => file.path));
  }
  ...
  getCreatedAndDeletedFiles(vaultFilePathsSet: Set<string>) {
    ...
    const createdFiles = Array.from(vaultFilePathsSet).filter((currentFile) => !this.lastVault.has(currentFile));
    const deletedFiles = Array.from(this.lastVault).filter((currentVaultFile) => !vaultFilePathsSet.has(currentVaultFile));
  ...
  getIndexFiles2BUpdated(changedFiles: string[]) {
    ...
    for (const changedFile of changedFiles) {
      ...
      if (indexFilePath && isInAllowedFolder(this.settings, indexFilePath)) {
  ...
  generateIndexContents = async (indexFile: string): Promise<void> => {
    ...
    let indexTFile = this.app.vault.getAbstractFileByPath(indexFile) || (await this.app.vault.create(indexFile, currentTemplateContent));

    if (indexTFile && indexTFile instanceof TFile) return this.generateIndexContent(indexTFile);
  };
  ...
  generateIndexContent = async (indexTFile: TFile): Promise<void> => {
    ...
    try {
      if (indexTFile instanceof TFile) {
        ...
        const updatedFrontmatter = hasFrontmatter(currentContent, this.settings.frontMatterSeparator) ? updateFrontmatter(this.settings, currentContent) : "";
  ...
  setEmojiPrefix = (isFile: boolean): string => {
    return this.settings.enableEmojis ? (isFile ? emoji.get(this.settings.fileEmoji) : emoji.get(this.settings.folderEmoji)) : "";
  };
  ...
  isIndexFile = (item: TAbstractFile): boolean => {
    return this.isFile(item) && item.name === `${this.settings.indexPrefix}${item.parent.name}`;
  };
